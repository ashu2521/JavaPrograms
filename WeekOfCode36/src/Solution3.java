...
//    private final boolean ONLINE_JUDGE = System.getProperty("ONLINE_JUDGE") != null;
    ...
    private void solve() {
        ...
        long answer = getAnswer(n, heights, costs);
        out.println(answer);
    }

    final static long INF = Long.MAX_VALUE / 10;

    static long getAnswer(int n, int[] heights, int[] costs) {
        heights[n] = Integer.MAX_VALUE;

        int[] nextHighers = new int[n + 1];
        nextHighers[n] = n;
        ...
        for (int i = n - 1; i >= 0; --i) {
            ...
            while (higher.size() > 0) {
                int nextIndex = higher.peek();
                if (height < heights[nextIndex]) {
                    nextHighers[i] = nextIndex;
                    break;
                } else {
                    higher.pop();
                }
        ...
        MinSegmentTree tree = new MinSegmentTree(n + 1, INF);

        for (int i = n - 1; i >= 0; --i) {
            int height = heights[i];

            int nextHigher = nextHighers[i];
            if (nextHigher == n) {
                dp[i] = min(dp[i], 0);
            } else {
                dp[i] = min(dp[i], dp[nextHigher] + heights[nextHigher] - height);
            }

            tree.getSegment(i + 1, nextHigher - 1, INF);
            dp[i] = min(dp[i], height + tree.result);

            dp[i] += costs[i];

            tree.updateIndex(i, dp[i] - height);
        }
    ...
    static class MinSegmentTree extends SegmentTree {
        ...
        MinSegmentTree(int size, long defaultValue) {
            super(size, false);
        ...
        void updateVertex(int v, int vLeft, int vRight) {
            tree[v] = min(tree[vLeft], tree[vRight]);
        }

        long result;

        @Override
        void initResult(long... initResultValues) {
            result = initResultValues[0];
        }

        @Override
        void accumulateResult(int v, int left, int right) {
